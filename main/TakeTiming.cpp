...
TakeTiming::spliceOffset() const
{
    ...
    sv_frame_t offset = latency + preRoll;
...
TakeTiming::autoStopFrames() const
{
    ...
    sv_frame_t margin = sv_frame_t(rate * autoStopMarginSeconds());
    return spliceOffset() + (end - position) + margin;
}
...
TakeTiming::liveFrameIntoTake(sv_frame_t recordedFrame) const
{
    ...
    return compensatedLiveFrame(recordedFrame, spliceOffset());
}
...
TakeTiming::isInLeadIn(sv_frame_t framesReceived) const
{
    ...
    return preRoll > 0 && framesReceived < spliceOffset();
}
...
TakeTiming::countdownSeconds(sv_frame_t framesReceived) const
{
    ...
    double seconds = double(spliceOffset() - framesReceived) / rate;
