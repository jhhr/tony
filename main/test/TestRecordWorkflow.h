...
private slots:
    ...
    void lyrics_import_disabled_while_recording() {
        ...
        // Something to analyse: the fake device delivers from a thread of
        // its own, and a take stopped as soon as it started can have no
        // audio in it, which is never analysed
        QTest::qWait(500);
        stopTake();
        if (QTest::currentTestFailed()) return;
        QTRY_VERIFY_WITH_TIMEOUT(import->isEnabled(), 2000);
