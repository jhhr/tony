 ...
 * its recording (besides the start gap, which is measured), the output
 * and input latencies the device reported, and the rate the device
 * recorded at.  All 0 until known; the round trip stays 0 for a take
 * made without the reference playing, which is placed with none.
 *
 ...
 * device.  The two kinds differ only when the device's rate is not the
 * session's, and the round trip then adds the one to the other.
 */
struct TakeLatency
{
    ...
    TakeLatency() : roundTrip(0), reportedOutput(0), reportedInput(0),
                    recordingRate(0) { }
